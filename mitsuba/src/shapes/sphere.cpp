...
public:
	...
	Point getCenter(Float time) const {
		const Transform& trafo = m_objectToWorld->eval(time);
		return trafo(Point(0,0,0));
	}

	void sampleDirectPoint(DirectSamplingRecord& dRec) override {
		const Transform& trafo = m_objectToWorld->eval(dRec.time);
		Point m_center = trafo(Point(0,0,0));
		...
		//dRec.pdf = m_invSurfaceArea * dRec.dist * dRec.dist / absDot(dRec.d, dRec.n);
		//dRec.pdf = 1.0;
	}

    Sphere(const Properties &props) : Shape(props) {
        m_objectToWorld =
            new AnimatedTransform(Transform::translate(Vector(props.getPoint("center", Point(0.0f)))));
        m_radius = props.getFloat("radius", 1.0f);

        if (props.hasProperty("toWorld")) {
            ref<AnimatedTransform> objectToWorld = new AnimatedTransform(props.getAnimatedTransform("toWorld"));
            //Float radius = objectToWorld(0.0, Vector(1,0,0)).length();
            Float radius = objectToWorld->eval(0.0)(Vector(1,0,0)).length();
    ...
    AABB getAABB() const {
		...
		for (std::set<Float>::iterator it = times.begin(); it != times.end(); ++it) {
			...
			Point3 center = trafo(Point(0,0,0));

			aabb.expandBy(center - Vector(m_radius));
			aabb.expandBy(center + Vector(m_radius));
		}
		return aabb;
    }
    ...
    void fillIntersectionRecord(const Ray &ray,
    	...
    	const Transform& trafo = m_objectToWorld->eval(ray.time);
    	Point m_center = trafo(Point(0,0,0));
    	const Transform& m_worldToObject = m_objectToWorld->eval(ray.time).inverse();
    ...
    ref<TriMesh> createTriMesh() {
        ...
        for (uint32_t theta=0; theta<thetaSteps; ++theta) {
            ...
            for (uint32_t phi=0; phi<phiSteps; ++phi) {
                ...
                vertices[vertexIdx] = m_objectToWorld->eval(0.0)(Point(v*m_radius));
                normals[vertexIdx++] = m_objectToWorld->eval(0.0)(Normal(v));
            }
        }
    ...
    std::string toString() const {
        ...
        oss << "Sphere[" << endl
            ...
            << "  objectToWorld = " << m_objectToWorld->toString() << "," << endl
