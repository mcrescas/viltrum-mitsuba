...
SceneHandler::SceneHandler(const ParameterMap &params,
    ...
    m_tags["animatedValue"] = TagEntry(EAnimationValue, (Class *) NULL);
...
void SceneHandler::startElement(const XMLCh* const xmlName,
    ...
    switch (tag.first) {
        ...
        case EAnimation: {
            ...
            break;
        case EAnimationValue: {
                m_animatedValue = new AnimatedValue();
            }
            break;
        default:
            break;
    ...
    switch (tag.first) {
        ...
        case EFloat: {
                ...
                if (m_animatedValue) {
                    ...
                    m_animatedValue->append(time, value);
                } else {
                ...
                if (hasValue == hasFilename) {
                    ...
                    if (tokens.size() == 1 && tokens[0].find(':') == std::string::npos) {
                        ...
                        if (tokens[0].find(':') != std::string::npos) {
                            ...
                            if (m_animatedValue) {
                                Float time = parseFloat("time", context.attributes["time"]);
                                m_animatedValue->append(time, Spectrum(value));
                            } else {
                                context.parent->properties.setSpectrum(context.attributes["name"],
                                Spectrum(value));
                            }
        ...
        case EAnimationValue: {
                //m_animatedTransform->sortAndSimplify();
                context.parent->properties.setAnimatedValue(
                    context.attributes["name"], m_animatedValue);
                m_animatedValue = NULL;
            }
            break;
        ...
        default: {
                ...
                if (tag.second == MTS_CLASS(Shape)
                    ...
                    && props.getPluginName() != "disk" && props.getPluginName() != "sphere")) {
