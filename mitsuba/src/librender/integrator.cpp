...
void SamplingIntegrator::renderBlock(const Scene *scene,
    ...
    for (size_t i = 0; i<points.size(); ++i) {
        ...
        mitsuba::Spectrum info_samples(0.0f);
        int n_samples = sampler->getSampleCount();
        ...
        //info_samples /= n_samples;
        //block->getBitmap()->setPixel(Point2i(points[i]), info_samples);
    }
}
...
MonteCarloIntegrator::MonteCarloIntegrator(const Properties &props) : SamplingIntegrator(props) {
    ...
    std::string modeMIS = props.getString("modeMIS", "all");
    if (modeMIS == "all") {
        m_modeMIS = MIS::ALL;
    } else if (modeMIS == "emitter") {
        m_modeMIS = MIS::EMITTER;
    } else if (modeMIS == "brdf") {
        m_modeMIS = MIS::BRDF;
    } else if (modeMIS == "emitter_weight") {
        m_modeMIS = MIS::EMITTER_WEIGHT;
    } else if (modeMIS == "brdf_weight") {
        m_modeMIS = MIS::BRDF_WEIGHT;
    } else {
        Log(EError, "[modeMIS] Type of MIS undefined  | Available : ALL , EMITTER , BRDF");
    }
}
