...
typedef boost::variant<
    bool, int64_t, Float, Point, Vector, Transform, AnimatedTransform *, AnimatedValue *,
 ...
 *      Custom
 */
void Properties::setAnimatedValue(const std::string &name, const AnimatedValue *value, bool warnDuplicates) {
    if (hasProperty(name)) {
        AnimatedValue **old = boost::get<AnimatedValue *>(&((*m_elements)[name].data));
        if (old)
            (*old)->decRef();
        if (warnDuplicates)
            SLog(EWarn, "Property \"%s\" was specified multiple times!", name.c_str());
    }
    (*m_elements)[name].data = (AnimatedValue *) value;
    (*m_elements)[name].queried = false;
    value->incRef();
}

ref<const AnimatedValue> Properties::getAnimatedValue(const std::string &name) const {
    std::map<std::string, PropertyElement>::const_iterator it = m_elements->find(name);
    if (it == m_elements->end())
        SLog(EError, "Property \"%s\" missing", name.c_str());
    AnimatedValue * const * result1 = boost::get<AnimatedValue *>(&it->second.data);
    const Float *result2 = boost::get<Float>(&it->second.data);
    const Spectrum *result3 = boost::get<Spectrum>(&it->second.data);

    if (!result1 && !result2 && !result3)
        SLog(EError, "The property \"%s\" has the wrong type (expected <animationValue> or <Float> or <Spectrum>). The "
                "complete property record is :\n%s", name.c_str(), toString().c_str());
    ...
    if (result1)
        ...
        return new AnimatedValue(*result2);
    else
        return new AnimatedValue(*result3);
}

ref<const AnimatedValue> Properties::getAnimatedValue(const std::string &name, const AnimatedValue *defVal) const {
    std::map<std::string, PropertyElement>::const_iterator it = m_elements->find(name);
    if (it == m_elements->end())
        return defVal;
    AnimatedValue * const * result1 = boost::get<AnimatedValue *>(&it->second.data);
...
ref<const AnimatedValue> Properties::getAnimatedValue(const std::string &name, const Float &defVal) const {
    std::map<std::string, PropertyElement>::const_iterator it = m_elements->find(name);
    if (it == m_elements->end())
        return new AnimatedValue(defVal);
...
ref<const AnimatedValue> Properties::getAnimatedValue(const std::string &name, const Spectrum &defVal) const {
    std::map<std::string, PropertyElement>::const_iterator it = m_elements->find(name);
    if (it == m_elements->end())
        return new AnimatedValue(defVal);
