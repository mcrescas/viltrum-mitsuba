...
public:
    ...
    Spectrum sampleDirectPoint(DirectSamplingRecord& dRec) const override {
        ...
        Float invDist = (dRec.dist>Epsilon)?(1.0f / dRec.dist):1.e6;
        if (dRec.dist>Epsilon) dRec.d *= invDist;
        else dRec.d=Vector3(0.0f,0.0f,1.0f);
        dRec.n = Normal(0.0f);
        dRec.pdf = 1;
        dRec.measure = EDiscrete;

        return m_intensity * ((dRec.dist>Epsilon)?falloffCurve(trafo.inverse()(-dRec.d)):Spectrum(1.0f)) * (invDist * invDist);
    }
