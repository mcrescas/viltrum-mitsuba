...
public:
    PointEmitter(const Properties &props) : Emitter(props) {
        ...
        m_intensity = new AnimatedValue(props.getAnimatedValue("intensity", Spectrum::getD65()));
    }
    ...
    Spectrum sampleDirectPoint(DirectSamplingRecord& dRec) const override {
        ...
        Spectrum intensity = m_intensity->eval(dRec.time);
    ...
    Spectrum samplePosition(PositionSamplingRecord &pRec, const Point2 &sample,
        ...
        Spectrum intensity = m_intensity->eval(pRec.time);
        return intensity * (4 * M_PI);
    }

    Spectrum evalPosition(const PositionSamplingRecord &pRec) const {
        Spectrum intensity = m_intensity->eval(pRec.time);
        return (pRec.measure == EDiscrete) ? (intensity * 4*M_PI) : Spectrum(0.0f);
    }
    ...
    Spectrum sampleRay(Ray &ray,
        ...
        // return m_intensity * (4 * M_PI);
        Spectrum intensity = m_intensity->eval(time);
        return intensity * (4 * M_PI);
    }
...
Shader *PointEmitter::createShader(Renderer *renderer) const {
    return new PointEmitterShader(renderer, m_intensity->eval(0.0f));
}
