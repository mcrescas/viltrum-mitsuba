...
public:
    ...
    mutable bool m_onlyAbsorption;
    mutable bool m_singleScattering;

    SimpleVolumetricPathTracer(const Properties &props) : MonteCarloIntegrator(props) {
        m_onlyAbsorption = props.getBoolean("onlyAbsorption", false);
        m_singleScattering = props.getBoolean("singleScattering", false);

        if (m_onlyAbsorption && m_singleScattering) {
            Log(EError, "Only one : onlyAbsorption vs singleScattering");
        }
    }
    ...
    Spectrum Li(const RayDifferential &r, RadianceQueryRecord &rRec) const {
        ...
        while (rRec.depth <= m_maxDepth || m_maxDepth < 0) {
            ...
            /* ==================================================================== */
            if (m_onlyAbsorption && !m_singleScattering && rRec.medium) {
                mRec.transmittance = rRec.medium->evalTransmittance(ray, rRec.sampler);
                //assert(mRec.transmittance.min() >= 0.0f);
                //assert(mRec.transmittance.max() <= 1.0);
                mRec.pdfFailure = 1.0;
            }
            if (!m_onlyAbsorption && rRec.medium && rRec.medium->sampleDistance(Ray(ray, 0, its.t), mRec, rRec.sampler)) {
                /* Sample the integral
                   \int_x^y tau(x, x') [ \sigma_s \int_{S^2} \rho(\omega,\omega') L(x,\omega') d\omega' ] dx'
                */
                const PhaseFunction *phase = rRec.medium->getPhaseFunction();

                // Copy because of single scattering
                Spectrum copyBeta = throughput;
                ...
                if (m_singleScattering) {
                    // m_onlyAbsorption = true;
                    // m_singleScattering = false;
                    // throughput = copyBeta;
                ...
                */
                ...
                if (rRec.medium)
                    //Log(EInfo, "Outside tr %f  | PDF %f", mRec.transmittance[0], mRec.pdfFailure);
