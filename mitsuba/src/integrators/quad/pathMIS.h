...
#ifndef SRC_INTEGRATORS_PATHMIS_H_
#define SRC_INTEGRATORS_PATHMIS_H_
...
static StatsCounter avgPathLengthMIS("Path tracer MIS", "Average path length", EAverage);
 ...
 * }
 */
class PathTracerMIS : public MonteCarloIntegrator {
public:

    PathTracerMIS(const Properties &props)
        : MonteCarloIntegrator(props) { }

    /// Unserialize from a binary data stream
    PathTracerMIS(Stream *stream, InstanceManager *manager)
    ...
    Spectrum Li(const RayDifferential &r, RadianceQueryRecord &rRec) const {
        ...
        // Update direction to avoid math issues
        ray.setDirection(ray.d + Vector(Epsilon));
        ...
        // Avoid first intersection with emitter when hiding it (but it still possible to conect with it)
        while (rRec.its.isValid() && rRec.its.isEmitter() && m_hideEmitters) {
            Point newOrigin = ray.o + ray.d * (rRec.its.t + Epsilon);
        ...
        while (rRec.depth <= m_maxDepth || m_maxDepth < 0) {
            ...
            if (its.isEmitter() && (rRec.type & RadianceQueryRecord::EEmittedRadiance)
                && (!m_hideEmitters || scattered)) {
                    if (m_modeMIS == MIS::EMITTER_WEIGHT || m_modeMIS == MIS::BRDF_WEIGHT) {
                        Li += throughput * its.Le(-ray.d) * 0.5;
                    } else {
            ...
            if (maxDepthLower != -1 && rRec.depth >= maxDepthLower) {
                // LiHigher = Li;
                // Li = Spectrum(0.0f);
            ...
            /*
             *      Uses the same two samples for MIS emitters and BRDF
             */
            Point2 sampleMIS = rRec.nextSample2D();
            if (sampleMIS.x == 0.0) sampleMIS.x += Epsilon;
            if (sampleMIS.y == 0.0) sampleMIS.y += Epsilon;
            if (sampleMIS.x == 1.0) sampleMIS.x -= Epsilon;
            if (sampleMIS.y == 1.0) sampleMIS.x -= Epsilon;
            ...
            if (rRec.type & RadianceQueryRecord::EDirectSurfaceRadiance &&
                ...
                Spectrum value = scene->sampleEmitterDirect(dRec, Point2(sampleMIS));
                if (!value.isZero()) {
                    const Emitter *emitter = static_cast<const Emitter *>(dRec.object);

                    /* Allocate a record for querying the BSDF */
                    BSDFSamplingRecord bRec(its, its.toLocal(dRec.d + Vector(Epsilon)), ERadiance);
                    ...
                    if (!bsdfVal.isZero() && (!m_strictNormals
                        ...
                        if (m_modeMIS == MIS::ALL) {
                            Li += throughput * value * bsdfVal * weight;
                        } else if (m_modeMIS == MIS::EMITTER) {
                            Li += throughput * value * bsdfVal;
                        } else if (m_modeMIS == MIS::EMITTER_WEIGHT) {
                            Li += throughput * value * bsdfVal * weight;
                        }
                    }
                }
            ...
            /* Revert to two-samples MIS when achieving higher order non-piecewise radiance */
            Spectrum bsdfWeight;
            if (!swapped_lightHigher) {
                bsdfWeight = bsdf->sample(bRec, bsdfPdf, Point2(sampleMIS));
            } else {
            ...
            if (hitEmitter &&
                ...
                } else if (m_modeMIS == MIS::BRDF) {
                    Li += throughput * value;
                } else if (m_modeMIS == MIS::BRDF_WEIGHT) {
                    Li += throughput * value * miWeight(bsdfPdf, lumPdf);
                }
            }
        ...
        /* Store statistics */
        avgPathLengthMIS.incrementBase();
        avgPathLengthMIS += rRec.depth;
