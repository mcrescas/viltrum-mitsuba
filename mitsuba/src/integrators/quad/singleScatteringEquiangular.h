...
#ifndef SRC_INTEGRATORS_SINGLESCATTERINGEQUIANGULAR_H_
#define SRC_INTEGRATORS_SINGLESCATTERINGEQUIANGULAR_H_
...
static StatsCounter avgPathLength_singleScattering_equiangular("Single scattering quadrature", "Average path length", EAverage);
 ...
 * }
 */
class SingleScatteringQuadratureEquiangular : public MonteCarloIntegrator {
public:
	SingleScatteringQuadratureEquiangular(const Properties &props)
	...
	std::tuple<Float,Float> sample_distance_equiangular(const Point lightPos, const Ray &ray, Float sample) const {
        ...
        Float r = math::lerp(std::min(1.0 - 1.e-20, std::max(1.e-20,sample)), thetaA, thetaB);
    ...
    Float pdf_equiangular(const Point lightPos, const Ray &ray, const Float dist) const{
        // distance from ray to light closest point in ray
        Float t_light = dot(lightPos - ray.o, ray.d);
    ...
    Float cdf_equiangular(const Point lightPos, const Ray &ray, const Float dist) const{
        // distance from ray to light closest point in ray
        Float t_light = dot(lightPos - ray.o, ray.d);
        Float t = dist - t_light;
	...
	SingleScatteringQuadratureEquiangular(Stream *stream, InstanceManager *manager)
    ...
    Spectrum Li(const RayDifferential &r, RadianceQueryRecord &rRec) const {
        ...
        while (rRec.depth <= m_maxDepth || m_maxDepth < 0) {
            if (rRec.medium) {
                ...
                // Get light position
                const Transform &trafo = emitters[0]->getWorldTransform()->eval(ray.time);
                Point lightPos = trafo.transformAffine(Point(0.0f));

                //Sampling
                Float medium_sample = rRec.nextSample1D();

                // [0, Ts] Mapping
                Float equiangular_t_sample = SingleScatteringSampling::cdf_equiangular(lightPos, ray, its.t);
                if(m_mapping_0_ts) medium_sample*=(equiangular_t_sample);

                // Equiangular sampling
                auto [equiangular_medium_distance, single_scattering_probability_factor] = SingleScatteringSampling::sample_distance_equiangular(lightPos, ray, medium_sample);

                if ((!its.isValid()) || m_mapping_0_ts ||  (equiangular_medium_distance < its.t)) {  //We hit with the medium so we calculate is contribution and NEVER HIT ANYTHING ELSE
                        throughput *= (-(equiangular_medium_distance)*rRec.medium->getSigmaT()).exp();

                         // [0, Ts] Mapping
                        if(m_mapping_0_ts)throughput *= equiangular_t_sample;

                        if (m_solid_angle_sampling && false) {
                            const PhaseFunction *phase = rRec.medium->getPhaseFunction();
                            MediumSamplingRecord mRec; mRec.t = equiangular_medium_distance; mRec.p = ray(mRec.t); mRec.medium = rRec.medium; mRec.time = ray.time;
                            ...
                            else return single_scattering_probability_factor * value * throughput * rRec.medium->getSigmaS()
                                            * phase->eval(PhaseFunctionSamplingRecord(mRec, -ray.d, dRec.d));
                        } else {
                            const ref_vector<Emitter>& emitters = scene->getEmitters();
                            const PhaseFunction *phase = rRec.medium->getPhaseFunction();

                            // Medium point
                            MediumSamplingRecord mRec; //mRec.t = equiangular_medium_distance; mRec.p = ray(mRec.t);
                            mRec.medium = rRec.medium; mRec.time = ray.time;
                            mRec.t = equiangular_medium_distance; mRec.p = ray(mRec.t);
                            ...
                            for (const ref<Emitter> & emitter : emitters) {
                                ...
                                if (!value.isZero()) {
                                    ...
                                    Li += single_scattering_probability_factor * value * throughput * rRec.medium->getSigmaS()
                                            * phase->eval(PhaseFunctionSamplingRecord(mRec, -ray.d, dRec.d));
                                }
                            }
                    ...
                    //throughput *= (-(its.t)*rRec.medium->getSigmaT()).exp()/(1.0 - SingleScatteringSampling::cdf_equiangular(lightPos, ray, its.t));
                    //throughput *= (-(its.t)*rRec.medium->getSigmaT()).exp()*single_scattering_probability_factor;
                    //throughput *= (-(its.t)*rRec.medium->getSigmaT()).exp()*single_scattering_probability_factor*std::exp(-(equiangular_medium_distance - its.t));
                    //throughput *= surface_hit_factor(*rRec.medium, equiangular_medium_distance);
                    // LATEST ONE
                    throughput = (-its.t*rRec.medium->getSigmaT()).exp() / (1.0 - SingleScatteringSampling::cdf_equiangular(lightPos, ray, its.t));
                }
        ...
        /* Store statistics */
        avgPathLength_singleScattering_equiangular.incrementBase();
        avgPathLength_singleScattering_equiangular += rRec.depth;
