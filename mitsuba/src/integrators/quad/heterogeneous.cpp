...
#include "tracking/cvquadrature_tracking.h"
#include "tracking/delta_tracking.h"
#include "tracking/quad_tracking.h"
...
// #define MEDIA_STATS
#if defined(MEDIA_STATS)
static StatsCounter QueriesMedia("Heterogeneous volume Quad",
        "Number of queries of the media", ENumberValue);
static StatsCounter avgQueriesMedia("Heterogeneous volume Quad",
        "Average number of queries of the media", EAverage);
#endif
 ...
 * \end{xml}
 */
class HeterogeneousMediumQuad : public Medium {
public:
    ...
    enum EIntegrationMethod {
         ...
         *
         */
        EResidualRatioTracking,

        /**
         *
         */
        EControlVariatesAdaptive,

        ESimpsonQuadAdaptive,
    ...
    HeterogeneousMediumQuad(const Properties &props)
        : Medium(props) {
        ...
        m_numberSamples = props.getInteger("numberSamples", 2);
        adaptive_iters = props.getInteger("adaptiveIters", 3);
        ...
        else if (method == "deltatracking")
            m_method = EDeltaTracking;
        else if (method == "ratiotracking")
            m_method = ERatioTracking;
        else if (method == "residualratiotracking")
            m_method = EResidualRatioTracking;
        else if (method == "cvadaptive")
            m_method = EControlVariatesAdaptive;
        else if (method == "adaptivesimpson")
            m_method = ESimpsonQuadAdaptive;
        else if (method == "adaptiveboole")
            m_method = EBooleQuadAdaptive;
        else
            Log(EError, "Unsupported integration method \"%s\"!", method.c_str());
    }

    /* Unserialize from a binary data stream */
    HeterogeneousMediumQuad(Stream *stream, InstanceManager *manager)
        ...
        m_numberSamples = stream->readInt();
    ...
    void serialize(Stream *stream, InstanceManager *manager) const {
        ...
        stream->writeInt(m_numberSamples);
    }
    ...
    void configure() {
        ...
        maj_sigma_t = m_maxDensity;
        c_sigma_t = m_scale * m_density->getAvgFloatValue();

        Log(EInfo, "Majorant %f    Control %f", maj_sigma_t, c_sigma_t);
    ...
    Spectrum evalTransmittance(const Ray &ray, Sampler *sampler) const {
        if (m_method == ESimpsonQuadrature || sampler == NULL) {
        ...
        } else if (m_method == EIntegrationMethod::EWoodcockTracking) {
            ...
            #if defined(MEDIA_STATS)
                avgQueriesMedia.incrementBase();
            #endif

            Float result = 0;

            for (int i=0; i<m_numberSamples; ++i) {
                Float t = mint;
                while (true) {
            ...
            return Spectrum(result/m_numberSamples);

        } else if (m_method == EIntegrationMethod::ERatioTracking ||
                   m_method == EIntegrationMethod::EResidualRatioTracking ||
                   m_method == EIntegrationMethod::EControlVariatesAdaptive ||
                   m_method == EIntegrationMethod::EDeltaTracking ||
                   m_method == EIntegrationMethod::ESimpsonQuadAdaptive ||
                   m_method == EIntegrationMethod::EBooleQuadAdaptive) {
            ...
            auto func = FunctionWrapperMedium(ray, mint, maxt, this);

            if (m_method == EIntegrationMethod::EDeltaTracking) {
                auto stepper = delta_tracking(std::forward<decltype(rng)>(rng), maj_sigma_t);
                return evalStepper(stepper, func, mint, maxt);
            } else if (m_method == EIntegrationMethod::ERatioTracking) {
                auto stepper = ratio_tracking(std::forward<decltype(rng)>(rng), maj_sigma_t);
                return evalStepper(stepper, func, mint, maxt);
            } else if (m_method == EIntegrationMethod::EResidualRatioTracking) {
                auto stepper = residual_ratio_tracking(std::forward<decltype(rng)>(rng), maj_sigma_t, c_sigma_t);
                return evalStepper(stepper, func, mint, maxt);
            } else if (m_method == EIntegrationMethod::EControlVariatesAdaptive) {
                auto stepper = adaptive_residual_ratio_tracking(nested(simpson, trapezoidal), adaptive_iters, std::forward<decltype(rng)>(rng), maj_sigma_t);
                return evalStepper(stepper, func, mint, maxt);
            } else if (m_method == EIntegrationMethod::ESimpsonQuadAdaptive) {
                auto stepper = adaptive_quad_tracking(nested(simpson, trapezoidal), adaptive_iters, std::forward<decltype(rng)>(rng));
                return evalStepper(stepper, func, mint, maxt);
            } else if (m_method == EIntegrationMethod::EBooleQuadAdaptive) {
                auto stepper = adaptive_quad_tracking(nested(boole, simpson), adaptive_iters, std::forward<decltype(rng)>(rng));
                return evalStepper(stepper, func, mint, maxt);
            }
        } else {
            Log(EError, "Heterogenous media estimator doesnt exists");
            return Spectrum(-1);
        }
        //return Spectrum(1.0);
    }

    Spectrum evalTr(const Ray& ray, Spectrum* estimatedSigmaS=nullptr) const {
        std::random_device rd;
        std::mt19937_64 rng (rd());

        auto func = FunctionWrapperMedium(ray, ray.mint, ray.maxt, this);

        if (m_method == EIntegrationMethod::EDeltaTracking) {
            auto stepper = delta_tracking(std::forward<decltype(rng)>(rng), maj_sigma_t);
            return evalStepper(stepper, func, ray.mint, ray.maxt, estimatedSigmaS);
        } else if (m_method == EIntegrationMethod::ERatioTracking) {
            auto stepper = ratio_tracking(std::forward<decltype(rng)>(rng), maj_sigma_t);
            return evalStepper(stepper, func, ray.mint, ray.maxt, estimatedSigmaS);
        } else if (m_method == EIntegrationMethod::EResidualRatioTracking) {
            ...
            auto stepper = adaptive_quad_tracking(nested(simpson, trapezoidal), adaptive_iters, std::forward<decltype(rng)>(rng));
            return evalStepper(stepper, func, ray.mint, ray.maxt);
        } else if (m_method == EIntegrationMethod::EBooleQuadAdaptive) {
            auto stepper = adaptive_quad_tracking(nested(boole, simpson), adaptive_iters, std::forward<decltype(rng)>(rng));
            return evalStepper(stepper, func, ray.mint, ray.maxt);
        } else {
    ...
    Spectrum evalStepper(STEPPER& stepper, F& f, Float mint, Float maxt, Spectrum* estimatedSigmaS=nullptr) const {
        std::array<double, 1> range_min, range_max;
        range_min.fill(mint);
        range_max.fill(maxt);
        auto Frange = range(range_min, range_max);

        auto data = stepper.init(f, Frange);
        ...
        for (int i=0; i<m_numberSamples; i++) {
            stepper.step(f, Frange, data);
        }

        #if defined(MEDIA_STATS)
            avgQueriesMedia += f.samples();
            QueriesMedia += f.samples();
        #endif

        if (estimatedSigmaS) {
            Spectrum res (0.0);
            auto& arrayT = f.positions();
            const auto& ray = f.getRay();
            for(unsigned int i=0; i<arrayT.size(); i++) {
                Float t = arrayT[i];
                Point3 p = ray(t);
                Spectrum density = Spectrum(this->lookupDensity(p, ray.d) * this->m_scale);
                Spectrum albedo = m_albedo->lookupSpectrum(p);
                res += albedo;
            }

            (*estimatedSigmaS) = res / Spectrum(arrayT.size());
        }

        return Spectrum(stepper.integral(f, data));
    }
...
protected:
    ...
    // Used in residual ratio tracking and adaptive residual ratio tracking
    Float maj_sigma_t;
    Float c_sigma_t;
    int adaptive_iters;

    class FunctionWrapperMedium {
    public:
        FunctionWrapperMedium(const Ray& _ray, Float _mint, Float _maxt, const HeterogeneousMediumQuad* _medium) : ray(_ray), mint(_mint), maxt(_maxt), medium(_medium), queries(0) {}

        Float operator() (const std::array<Float, 1>& pos) const {
            //Float t = mint + pos[0] * (maxt - mint);
            const Float t = pos[0];
            const Point p = ray(t);
            Float density = medium->lookupDensity(p, ray.d) * medium->m_scale;
            queries++;
            arrayT.push_back(t);
            return density;
        }
    ...
    private:
        const Ray ray;
        Float mint, maxt;
        const HeterogeneousMediumQuad* medium;
        mutable std::vector<Float> arrayT;

        mutable unsigned long queries;
    };
};

MTS_IMPLEMENT_CLASS_S(HeterogeneousMediumQuad, false, Medium)
MTS_EXPORT_PLUGIN(HeterogeneousMediumQuad, "Heterogeneous medium Primary-Space Adaptive Control Variates");
