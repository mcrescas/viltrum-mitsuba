...
/*
 *  Save here only one track. 
 *  Save as abstract and recover later type when needed
 *  launch eval and return
 */

class MTS_EXPORT_CORE AnimatedValue : public Object {
public:

    class AnimatedValueProxy {
        public:
            AnimatedValueProxy(const AnimatedValue* _value, Float _t) : value(_value), t(_t) {}

            operator Float() const {
                return value->eval_Float(t);
            }

            operator Spectrum() const {
                return value->eval_Spectrum(t);
            }
        private:
    ...
    /*
     *  Return proxy to obtain the right value
     */
    inline AnimatedValueProxy eval(Float t) const {
        return AnimatedValueProxy(this, t);
    }

    inline Float eval_Float(Float t) const {
        return static_cast<const FloatTrack *>(track.get())->eval(t);
    }

    inline Spectrum eval_Spectrum(Float t) const {
        return static_cast<const SpectrumTrack *>(track.get())->eval(t);
    }

    template <typename T>
    inline void append(Float time, T value) {
        // Nullptr
        if (!track) {
            track = new AnimationTrack<T>(AbstractAnimationTrack::EAnimatedValue);
        }
        static_cast<AnimationTrack<T> *>(track.get())->append(time, value);
    }

    AnimatedValue(Spectrum v) {
        this->append(Float(0), v);
    }

    AnimatedValue(Float v) {
        this->append(Float(0), v);
    }

    AnimatedValue() {}

    AnimatedValue(const AnimatedValue *trafo) {
        track = trafo->track;
    }
...
private:
    ref<AbstractAnimationTrack> track;
};
