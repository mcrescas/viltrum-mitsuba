...
public:
    ...
    // Save higher order radiance (from bounces up to maxDepthLower)
    // mutable is needed because signature of Li is declared class const
    mutable Spectrum LiHigher;
    // Depth from where to start saving radiance in higher order buffer 
    int maxDepthLower = -1;
    ...
    // Default sample with both strategies
    MIS m_modeMIS;
